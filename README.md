...
予測時には、今回の最新ニュース要約に加えて次の2種類のコンテキストも与えます。

- **ニュースの文脈**: キーワードごとの過去(直近1日/1週間/1か月)のニュース要約をさらにLLMで要約したもの。`predictions/context/<キーワード>_<期間>.md` にキャッシュされ、元のニュースが変わっていない限り再生成されません。
- **株価の推移**: 日経平均・S&P500それぞれの直近1日/1週間/1か月の騰落率・高値安値レンジ(yfinanceの日足OHLCVから算出)。

明日の予測には直近1日、1週間後の予測には直近1週間、1か月後の予測には直近1か月のコンテキストを重視するようモデルに指示しています。
...
## 暦注とニュースを組み合わせた株価予測ログ(calendar_predictor.py、実験的機能)

**六曜・一粒万倍日・天赦日**といった日本の伝統的な暦注と、直近のニュース要約(全キーワード分)の両方を手がかりに、「明日」の株価を予測する実験的な機能です。

```
python calendar_predictor.py
```

本日・明日の暦注(六曜・一粒万倍日・天赦日・干支・季節区分)を`calendar_info.py`で算出し、`output/`の直近のニュース要約と合わせて`prediction_models.json`の各モデルに提示、暦注とニュースの両方を踏まえて「明日」の日本株式市場・米国株式市場への影響を独立に予測させ、多数決で最終判定します(1週間後・1か月後は対象外)。予測・答え合わせの仕組みは`predictor.py`と共通です。ログは`predictions/calendar_log.json`・`predictions/calendar_by_model/`に別系統で保存され、`site_generator.py`実行後は`docs/calendar.html`(ナビゲーションの「暦注予測(実験)」)で確認できます。

**これらの暦注は科学的根拠のない伝統的な考え方であり、実際の相場変動との因果関係は確認されていません。** あくまで実験・話のタネとしての機能です。
